...
export const postTrip = async (req, res, next) => {
  try {
    const { route_id, origin, destination, vehicle_id, driver_id, departure_time, arrival_time, price_per_seat } = req.body;
    ...
    const trip = await createTrip({
      ...
      origin: origin ? origin.trim() : 'Addis Ababa',
      destination: destination ? destination.trim() : 'Hawassa',
...
export const getTrips = async (req, res, next) => {
  try {
    ...
    const trips = await getAllTrips({ origin, destination, time });
